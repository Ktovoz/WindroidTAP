...
def get_corners(src_img: str, back_img: str, show: bool = False) -> Optional[List[Tuple[int, int]]]:
    ...
    try:
        ...
        _, _, dst = load_and_process_images(src_img, back_img)
        corners = [(int(x), int(y)) for x, y in dst.reshape(-1, 2)]
        if show:
            visualize_results(back_img, corners, show)
...
def get_center_location(src_img, back_img, show=False):
    ...
    try:
        ...
        visualize_results(back_img, [(x, y), (x + w, y), (x + w, y + h),
                                     (x, y + h)], show)
        return point
    except Exception as e:
        print(f"发生错误: {e}")
...
def save_target_capture(src_img, back_img, name='target'):
    ...
    try:
        # 捕获目标图像
        target_image = capture_target_image_v1(src_img, back_img)
...
def compare_image_v3(source_image_path, back_image_path, show=False):
    ...
    try:
        ...
        target_image = capture_target_image_v1(source_image_path,
                                               back_image_path)
...
if __name__ == '__main__':
    ...
    img = crop_image_by_corners(src, corners)
