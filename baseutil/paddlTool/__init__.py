...
def filter_ocr_results(img_path, confidence_threshold=0.8):
    """
...
def process_image(img1, result, font_path=None):
    """
    处理图像并保存结果。
...
def filter_and_select_text(img_path, pattern, pattern_type='char'):
    """
    ...
    try:
        ...
        data = filter_ocr_results(img_path)
        if not data:
